...
model = YOLIC_MobileNetV2(model_id=7, project_dir="./")
...
def train(epoch, model, loss_fn, ce_loss_fn):
    ...
    for batch_idx, (data, new_labelForBranch1, new_labelForBranch2, LabelImage) in enumerate(train_loader):
        ...
        # with autocast():
        output_branch1, Feature_branch1 = model(data, "branch1")  # Feature.shape=torch.Size([batch,64,375,1242])
        # print(Feature_branch1.shape)
        # print(output_branch1.shape, new_labelForBranch1.shape)  # torch.Size([2, 20, 5, 6]) torch.Size([2, 20, 5, 6])
    ...
    branch1_epoch_losses_train.append(branch1_epoch_loss)
    with open("%s/epoch_losses_train_branch1.pkl" % model.model_dir, "wb") as file:
        pickle.dump(branch1_epoch_losses_train, file)
    print("branch1_train loss: %g" % branch1_epoch_loss)
    plt.figure(1)
    plt.plot(branch1_epoch_losses_train, "k^")
    plt.plot(branch1_epoch_losses_train, "k")
    plt.ylabel("loss")
    plt.xlabel("epoch")
    plt.title("branch1_train loss per epoch")
    plt.savefig("%s/branch1_epoch_losses_train.png" % model.model_dir)
    plt.close(1)

    branch2_epoch_loss = np.mean(branch2_batch_losses)
    branch2_epoch_losses_train.append(branch2_epoch_loss)
    with open("%s/epoch_losses_train_branch2.pkl" % model.model_dir, "wb") as file:
        pickle.dump(branch2_epoch_losses_train, file)
    print("branch2_train loss: %g" % branch2_epoch_loss)
    plt.figure(1)
    plt.plot(branch2_epoch_losses_train, "k^")
    plt.plot(branch2_epoch_losses_train, "k")
    plt.ylabel("loss")
    plt.xlabel("epoch")
    plt.title("branch2_train loss per epoch")
    plt.savefig("%s/branch2_epoch_losses_train.png" % model.model_dir)
    plt.close(1)

    branch3_epoch_loss = np.mean(branch3_batch_losses)
    branch3_epoch_losses_train.append(branch3_epoch_loss)
    with open("%s/epoch_losses_train_branch3.pkl" % model.model_dir, "wb") as file:
        pickle.dump(branch3_epoch_losses_train, file)
    print("branch3_train loss: %g" % branch3_epoch_loss)
    plt.figure(1)
    plt.plot(branch3_epoch_losses_train, "k^")
    plt.plot(branch3_epoch_losses_train, "k")
    plt.ylabel("loss")
    plt.xlabel("epoch")
    plt.title("branch3_train loss per epoch")
    plt.savefig("%s/branch3_epoch_losses_train.png" % model.model_dir)
    plt.close(1)
...
def train_evaluate(loss_fn, ce_loss_fn):
    ...
    for batch_idx, (imgs, new_labelForBranch1, new_labelForBranch2, LabelImage) in enumerate(test_loader):
        with torch.no_grad():  # (corresponds to setting volatile=True in all variables, this is done during inference to reduce memory consumption)
            ...
            output_branch1, Feature_branch1 = model(imgs, "branch1")  # Feature.shape=torch.Size([batch,64,375,1242])
            # print(Feature_branch1.shape)
...
if __name__ == '__main__':
    epoch_losses_train = []
    branch1_epoch_losses_train = []
    branch2_epoch_losses_train = []
    branch3_epoch_losses_train = []
    epoch_losses_val = []
