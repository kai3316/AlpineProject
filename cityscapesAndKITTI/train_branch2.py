...
model = DeepLabV3(model_id=3, project_dir="./")
model_id = "3"
...
def train(epoch, model, loss_fn, ce_loss_fn):
    ...
    for batch_idx, (data, new_labelForBranch1, new_labelForBranch2, LabelImage) in enumerate(train_loader):
        ...
        point_x = branch2Setting["x"] * 75
        point_y = branch2Setting["y"] * 207
        # print(x.shape)
        # print(point_x, point_y)
        interesting_region = data[:, :, point_x: point_x + 75, point_y: point_y + 207]
        out_branch2 = model(interesting_region)  # torch.Size([2, 20, 5, 9])
        ...
        loss = loss_fn(out_branch2, Label_branch2)
        ...
        branch2_batch_losses.append(loss.data.cpu().numpy())
...
def train_evaluate(loss_fn, ce_loss_fn):
    ...
    for batch_idx, (imgs, new_labelForBranch1, new_labelForBranch2, LabelImage) in enumerate(test_loader):
        with torch.no_grad():  # (corresponds to setting volatile=True in all variables, this is done during inference to reduce memory consumption)
            ...
            # print(x.shape)
            # print(point_x, point_y)
            interesting_region = imgs[:, :, point_x: point_x + 75, point_y: point_y + 207]
            out_branch2 = model(interesting_region)  # torch.Size([2, 20, 5, 9])
            Label_branch2 = new_labelForBranch2[:, branch2Setting["x"] * 5 + branch2Setting["y"], :, :, :]
            # print(out_branch2.shape, Label_branch2.shape)
            # print(out_branch2.shape, new_labelForBranch2.shape)  # torch.Size([2, 20, 5, 9]) torch.Size([2, 30, 20, 5, 9])
