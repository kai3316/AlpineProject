...
# from resnet_branch_network import resnet_branch_network
# model = resnet_branch_network(branch1_classes=384, branch2_classes=384, branch3_classes=384, branch4_classes=96)
from ownmobilenetv2 import BranchMobileNetV2
...
save_name = 'multibranch_mobilenet_34294_fp16_with_teacher_newtest_withweight'
...
checkpoint = torch.load(os.path.join(os.getcwd(), "mobilenetv2_fp16_34294rgbd_1248output.pth.tar"))
...
model1.classifier[1] = nn.Linear(1280, 384)
model1.features[0][0] = nn.Conv2d(4, 32, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), bias=False)

model2 = models.mobilenet_v2()
model2.classifier[1] = nn.Linear(1280, 384)
model2.features[0][0] = nn.Conv2d(4, 32, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), bias=False)

model3 = models.mobilenet_v2()
model3.classifier[1] = nn.Linear(1280, 384)
model3.features[0][0] = nn.Conv2d(4, 32, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), bias=False)

model4 = models.mobilenet_v2()
model4.classifier[1] = nn.Linear(1280, 96)
model4.features[0][0] = nn.Conv2d(4, 32, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), bias=False)

checkpoint1 = torch.load(os.path.join(os.getcwd(), "mobilenetv2_fp16_34294rgbd_upperarea0-384_bce_batch32.pth.tar"))
checkpoint2 = torch.load(os.path.join(os.getcwd(), "mobilenetv2_fp16_34294rgbd_middle384-768_bce_batch32.pth.tar"))
checkpoint3 = torch.load(os.path.join(os.getcwd(), "mobilenetv2_fp16_34294rgbd_1248_768-1152with_original_checkpoint.pth.tar"))
checkpoint4 = torch.load(os.path.join(os.getcwd(), "mobilenetv2_fp16_34294rgbd_1248_1152_1248_with_original_checkpoint.pth.tar"))
...
def train(epoch, model, model1, model2, model3, model4, loss_fn):
    model.train()
    model1.eval()
    model2.eval()
    model3.eval()
    ...
    for batch_idx, (data, target1, target2, target3, target4) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        teacher_out1 = model1(data)
        teacher_out2 = model2(data)
        teacher_out3 = model3(data)
        teacher_out4 = model4(data)
        with autocast():
            ...
            KD_out1 = loss_fn(teacher_out1, target1)
            KD_out2 = loss_fn(teacher_out2, target2)
            KD_out3 = loss_fn(teacher_out3, target3)
            KD_out4 = loss_fn(teacher_out4, target4)

            loss = loss1_out + loss2_out + loss3_out + loss4_out + KD_out1 + KD_out2 + KD_out3 + KD_out4
            # print(loss)
...
if __name__ == '__main__':
    ...
    for epoch in range(1, args.epochs + 1):
        train(epoch, model, model1, model2, model3, model4, loss_fn=criterion)
        if epoch > 150:
