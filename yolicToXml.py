...
def create_tree(image_name, h, w):
    ...
    path.text = '{}{}'.format(image_dir, image_name)  # 用于返回当前工作目录
...
LABELS = ["Bump", "Column", "Dent", "Fence", "Creature", "Vehicle", "Wall", "Weed", "ZebraCrossing", "TrafficCone",
          "TrafficSign"]
for label_file in label_files:
    if label_file.endswith(".txt"):
        annotation = create_tree(label_file.strip('.txt')+".png", image_height, image_width)
        path_label = os.path.join(label_dir, label_file)
        label = np.loadtxt(path_label, dtype=np.int32)
        tree = ET.ElementTree(annotation)
        for cell in range(0, 1248, 12):
            cellObjects = label[cell:cell + 12]
            numCell = int(cell / 12)
            if not (cellObjects == normal).all():
                for i in range(12):
                    if cellObjects[i] == 1:
                        ...
                        ET.SubElement(bndbox, "xmin").text = str(box_list[numCell][0][0])
                        ET.SubElement(bndbox, "ymin").text = str(box_list[numCell][0][1])
                        ET.SubElement(bndbox, "xmax").text = str(box_list[numCell][1][0])
                        ET.SubElement(bndbox, "ymax").text = str(box_list[numCell][1][1])

        tree.write('{}{}.xml'.format(xml_dir, label_file.strip('.txt')))
