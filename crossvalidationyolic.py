...
# x_train, x_test = train_test_split(all_list, test_size=0.3, random_state=2)
...
if __name__ == '__main__':
    ...
    all_list = os.listdir(rgbdir)
    random.seed(1)
    random.shuffle(all_list)
    k_fold = 5
    total_size = len(all_list)
    fraction = 1 / k_fold
    seg = int(total_size * fraction)
    all_data = MultiLabelDataSet(rgbdir, depthdir, all_list, labeldir, trans, 0)
    for i in range(k_fold):
        ...
        train_set = torch.utils.data.dataset.Subset(all_data, train_indices)
        val_set = torch.utils.data.dataset.Subset(all_data, val_indices)
        ...
        save_name = model_name + "_5fold_cv_" + str(i)
        print(save_name)
        all_train_loss = []
        all_train_acc = []
        all_test_loss = []
        ...
        list_res = []
        for j in range(len(all_train_loss)):
            list_res.append([all_train_loss[j], all_train_acc[j], all_test_loss[j], all_test_acc[j]])
