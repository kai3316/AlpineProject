...
class MobileNetV2_dynamicFPN(nn.Module):
    def __init__(self, width_mult=1.):
        ...
        self.first_layer = nn.Sequential(
            nn.Conv2d(4, self.input_channel, kernel_size=3, stride=2, padding=1, bias=False),
        ...
        self.top_layer = nn.Conv2d(
            ...
            512, kernel_size=1, stride=1, padding=0)
        print(int(self.inverted_residual_setting[-1]['width_factor'] * self.width_mult))
        # Lateral layers
        ...
        self.lateral_setting = [setting for setting in self.inverted_residual_setting[:-1]
                                if setting['stride'] > 1]
        # print(self.lateral_setting)
        self.lateral_layers = nn.ModuleList([
            nn.Conv2d(int(setting['width_factor'] * self.width_mult),
                      512, kernel_size=1, stride=1, padding=0)
            for setting in self.lateral_setting])
        for setting in self.lateral_setting:
            print(int(setting['width_factor'] * self.width_mult))
        # Smooth layers
        # n = lateral layers + 1 for top layer
        self.smooth_layers = nn.ModuleList([nn.Conv2d(512, 1248, kernel_size=3, stride=1, padding=1)])
        for _ in range(len(self.lateral_layers)):
            self.smooth_layers.append(nn.Conv2d(512, 64, kernel_size=3, stride=1, padding=1))
        # self.outputs_layers = nn.ModuleList([nn.AdaptiveAvgPool2d((1, 1))] *
    ...
    def forward(self, x):
        ...
        # p_layers = [outputs_layers(torch.squeeze(p_layers, 0)).squeeze() for outputs_layers, p_layers in
        #             zip(self.outputs_layers, p_layers)]
        return p_layers
...
def test():
    ...
    # print(net)
    output = net(torch.randn(1, 4, 224, 224))
