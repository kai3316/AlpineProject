...
from BranchMobileNetV2 import mobilenet_v2
import os
class branch_network(nn.Module):
    def __init__(self, branch1_classes=384, branch2_classes=384, branch3_classes=384, branch4_classes=96):
        super(branch_network, self).__init__()
        ...
        self.mbl1 = nn.Linear(2048, branch1_classes)  # 0m-1m risk detector
        self.mbf2 = rn.layer4  # 1m-2m risk detector
        self.mbl2 = nn.Linear(2048, branch2_classes)  # 1m-2m risk detector
        self.mbf3 = rn.layer4  # 2m-6m risk detector
        self.mbl3 = nn.Linear(2048, branch3_classes)  # 2m-6m risk detector
        self.mbf4 = rn.layer4  # 2m-6m risk detector
        self.mbl4 = nn.Linear(2048, branch4_classes)  # 2m-6m risk detector
    ...
    def forward(self, x):
        ...
        feature_branch1_out = self.mbf1(common_out)

        branch1_out = self.avgpool(feature_branch1_out)
        branch1_out = torch.flatten(branch1_out, 1)
        branch1_out = self.mbl1(branch1_out)

        feature_branch2_out = self.mbf2(common_out)
        branch2_out = self.avgpool(feature_branch2_out)
        branch2_out = torch.flatten(branch2_out, 1)
        branch2_out = self.mbl2(branch2_out)

        feature_branch3_out = self.mbf3(common_out)
        branch3_out = self.avgpool(feature_branch3_out)
        branch3_out = torch.flatten(branch3_out, 1)
        branch3_out = self.mbl3(branch3_out)

        feature_branch4_out = self.mbf4(common_out)
        branch4_out = self.avgpool(feature_branch4_out)
